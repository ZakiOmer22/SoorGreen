<%@ Page Title="My Reports" Language="C#" MasterPageFile="~/Pages/Citizen/Site.Master"
    AutoEventWireup="true" Inherits="SoorGreen.Admin.MyReports" CodeBehind="MyReports.aspx.cs" %>
...
<asp:Content ID="MainContent" ContentPlaceHolderID="MainContent" runat="server">
    <div class="reports-container">
        ...
        <div class="d-flex justify-content-between align-items-center mt-4">
            <div class="text-muted">
                Showing
                <asp:Label ID="lblStartCount" runat="server" Text="1"></asp:Label>
                - 
                <asp:Label ID="lblEndCount" runat="server" Text="10"></asp:Label>
                of 
                ...
                <div class="modal-body-glass">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <!-- AI Classification -->
                            <div class="form-group" id="aiClassificationSection" style="display: none;">
                                <div class="alert alert-info">
                                    <h5><i class="fas fa-robot"></i>AI Waste Classification</h5>
                                    <div id="aiResult">
                                        <!-- AI results will appear here -->
                                    </div>
                                    <button type="button" class="btn btn-sm btn-primary mt-2" onclick="classifyWaste()">
                                        <i class="fas fa-magic"></i>Auto-Classify Waste
                                    </button>
                                </div>
                            </div>
    ...
    <script type="text/javascript">
        ...
        // Show AI section when image is selected
        function onImageSelected() {
            var fileUpload = document.getElementById('<%= fuWasteImage.ClientID %>');
            var aiSection = document.getElementById('aiClassificationSection');
        ...
        function classifyWaste() {
            ...
            aiResult.innerHTML = '<div class="text-center"><i class="fas fa-spinner fa-spin"></i> Analyzing waste image...</div>';
            ...
            xhr.open('POST', 'ReportWaste.aspx/ClassifyWasteImage', true);
                ...
                if (xhr.readyState === 4) {
                    if (xhr.status === 200) {
                        ...
                        if (response.d && response.d.success) {
                            aiResult.innerHTML = '<div class="text-success">' +
                                '<i class="fas fa-check-circle"></i> Detected: <strong>' +
                                response.d.category + '</strong><br>' +
                                '<small>Suggested waste type: ' + response.d.wasteType + '</small>' +
                                '</div>';

                            // Auto-select the waste type in dropdown if possible
                            autoSelectWasteType(response.d.wasteType);
                        } else {
                            aiResult.innerHTML = '<div class="text-warning">' +
                                '<i class="fas fa-exclamation-triangle"></i> ' +
                                (response.d.message || 'Classification failed') +
                                '</div>';
                        }
                    } else {
                        aiResult.innerHTML = '<div class="text-danger">' +
                            '<i class="fas fa-times-circle"></i> Error connecting to AI service' +
                            '</div>';
                    }
                }
            };
        ...
        // Auto-select waste type in dropdown
        function autoSelectWasteType(wasteType) {
            var ddlWasteType = document.getElementById('<%= ddlWasteType.ClientID %>');
            if (ddlWasteType) {
                for (var i = 0; i < ddlWasteType.options.length; i++) {
                    if (ddlWasteType.options[i].text.toLowerCase().indexOf(wasteType.toLowerCase()) !== -1) {
                        ddlWasteType.selectedIndex = i;
                        break;
                    }
                }
            }
        ...
        window.onload = function () {
            var fileUpload = document.getElementById('<%= fuWasteImage.ClientID %>');
            if (fileUpload) {
                fileUpload.addEventListener('change', onImageSelected);
            }
        };
